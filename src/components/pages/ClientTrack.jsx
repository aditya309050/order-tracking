...
import { Search, Clock, Cog, CheckCircle2, Package, Truck, CheckCheck, MapPin, Phone, ShieldCheck, Printer, Copy, Check, FileText, AlertCircle } from 'lucide-react';
import { fetchOrderById } from '@/services/api';
import { socket, subscribeToOrder, unsubscribeFromOrder } from '@/services/socket';
import StatusBadge, { STATUS_CONFIG } from '@/components/StatusBadge';

const TRACKING_STEPS = [
  { key: 'CONFIRMED', title: 'Order Confirmed', icon: Clock },
  { key: 'IN_PROCESS', title: 'In Production', icon: Cog },
  { key: 'COMPLETED', title: 'QC Inspection', icon: CheckCircle2 },
  { key: 'PACKED', title: 'Staged for Transit', icon: Package },
  { key: 'OUT_FOR_DELIVERY', title: 'Out for Delivery', icon: Truck },
  { key: 'DELIVERED', title: 'Fulfilled', icon: CheckCheck },
];

export default function ClientTrack(props) {
  ...
  useEffect(() => {
    ...
    return () => unsubscribeFromOrder(activeOrderNumber);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [activeOrderNumber]);

  useEffect(() => {
    const handleLiveStatus = (updatedOrder) => {
      if (currentOrder && (updatedOrder.order_number?.toUpperCase() === currentOrder.order_number?.toUpperCase() || updatedOrder.id === currentOrder.id)) {
        setCurrentOrder((prev) => ({ ...prev, ...updatedOrder }));
        setLiveToast({ status: updatedOrder.status, time: new Date().toLocaleTimeString() });
  ...
  const handleSearchSubmit = (e) => {
    e.preventDefault();
    if (searchQuery.trim()) router.push(`/track/${searchQuery.trim().toUpperCase()}`);
  };
  ...
  const currentStepIndex = currentOrder ? TRACKING_STEPS.findIndex((s) => s.key === currentOrder.status) : -1;

  const activityMap = {};
  if (currentOrder && currentOrder.timeline) {
    for (const act of currentOrder.timeline) activityMap[act.status_to] = act;
  }
    ...
    <div className="max-w-4xl mx-auto space-y-6 pb-20">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-2">
        <form onSubmit={handleSearchSubmit} className="relative flex-1 max-w-md">
          ...
          />
          <button type="submit" className="absolute right-1 top-1/2 -translate-y-1/2 px-2.5 py-1 text-[11px] font-medium bg-slate-800 hover:bg-slate-700 text-slate-200 rounded border border-slate-700 transition-colors">
            Track
          </button>
        </form>
          ...
          {['ORD-1025', 'ORD-1024', 'ORD-1021'].map((id) => (
            <button
              ...
              className={`text-[11px] font-mono px-2 py-1 rounded transition-colors ${
                activeOrderNumber === id ? 'bg-blue-950 text-blue-300 border border-blue-800/50 font-semibold' : 'bg-[#11141c] text-slate-400 hover:text-slate-200 border border-[#1f2533]'
              }`}
            >
              {id}
            </button>
      ...
      {liveToast && (
        <div className="p-3 rounded-md bg-blue-950/80 border border-blue-800/60 text-blue-200 text-xs flex items-center justify-between shadow-sm">
      ...
      {loading ? (
        ...
        <div className="p-10 text-center rounded-lg bg-[#11141c] border border-rose-900/40">
          ...
          <button onClick={() => router.push('/track/ORD-1025')} className="px-3.5 py-1.5 rounded-md bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-medium border border-slate-700">
          ...
          <div className="rounded-lg bg-[#11141c] border border-[#1f2533] p-6 shadow-sm">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-5 border-b border-[#1f2533]">
              <div>
                ...
                <div className="flex items-center gap-3 mt-1">
                  <h1 className="text-2xl font-bold font-mono text-white tracking-tight">{currentOrder.order_number}</h1>
              ...
              <div className="flex sm:flex-col sm:items-end justify-between items-center gap-2">
                <div className="text-left sm:text-right">
                  ...
                  <span className="text-sm font-semibold text-slate-200 font-mono">{currentOrder.estimated_delivery || 'Scheduling'}</span>
                </div>
                <div className="flex items-center gap-1.5 mt-1">
                  <button onClick={handleCopyLink} className="flex items-center gap-1 px-2.5 py-1 rounded text-[11px] font-medium bg-[#161a24] hover:bg-slate-800 text-slate-300 border border-[#1f2533] transition-colors">
                    {copied ? <Check size={12} className="text-emerald-400" /> : <Copy size={12} />}
                    <span>{copied ? 'Copied' : 'Share URL'}</span>
                  </button>
                  <button onClick={() => window.print()} className="flex items-center gap-1 px-2.5 py-1 rounded text-[11px] font-medium bg-[#161a24] hover:bg-slate-800 text-slate-300 border border-[#1f2533] transition-colors">
                    <Printer size={12} />
                    <span>Print</span>
                  </button>
                </div>
            ...
            <div className="pt-6 pb-2">
              <div className="relative">
                ...
                <div className="absolute top-4 left-4 h-0.5 bg-blue-500 hidden sm:block transition-all duration-500" style={{ width: `${Math.min(100, (currentStepIndex / (TRACKING_STEPS.length - 1)) * 92)}%` }} />
                      ...
                      <div key={step.key} className="flex flex-col items-start sm:items-center text-left sm:text-center group">
                        <div
                          className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-semibold mb-2 transition-colors border ${
                            isCompleted ? 'bg-blue-600 border-blue-500 text-white' : isCurrent ? 'bg-[#11141c] border-blue-400 text-blue-400 ring-4 ring-blue-500/20' : 'bg-[#11141c] border-[#1f2533] text-slate-600'
                          }`}
                        >
                          {isCompleted ? <Check size={14} className="stroke-[2.5]" /> : <StepIcon size={14} />}
                        </div>
                        <span className={`text-xs font-semibold leading-tight ${isCurrent ? 'text-white' : isCompleted ? 'text-slate-300' : 'text-slate-500'}`}>{step.title}</span>
                        <span className="text-[10px] text-slate-500 font-mono mt-0.5">{act ? act.created_at?.slice(5, 16) : isCurrent ? 'Active Now' : 'Pending'}</span>
                      </div>
                    );
                  })}
                </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="md:col-span-2 rounded-lg bg-[#11141c] border border-[#1f2533] p-5 space-y-4">
              <div className="flex items-center justify-between pb-3 border-b border-[#1f2533]">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-xs font-bold uppercase tracking-wider text-slate-300">Production Item Manifest</h3>
                </div>
                <span className="text-[11px] font-mono text-slate-400">{currentOrder.quantity} Units Total</span>
              </div>
              <div className="rounded-md bg-[#090b10] border border-[#1f2533] p-4 space-y-3">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <h4 className="text-sm font-semibold text-white">{currentOrder.product}</h4>
                    <p className="text-xs text-slate-400 mt-0.5">Category: {currentOrder.category} &bull; Dimensions: {currentOrder.dimensions || 'Custom Size'}</p>
                  </div>
                  <div className="text-right shrink-0">
                    <span className="text-xs font-mono font-bold text-white bg-slate-800 px-2 py-0.5 rounded border border-slate-700">QTY: {currentOrder.quantity}</span>
                  </div>
                </div>
                {currentOrder.design_notes && (
                  <div className="pt-2 border-t border-[#1f2533] text-xs">
                    <span className="text-slate-500 text-[11px] font-medium block mb-1">Engineering &amp; Fabrication Notes:</span>
                    <p className="text-slate-300 font-mono text-[11px] leading-relaxed bg-[#11141c] p-2.5 rounded border border-[#1f2533]">{currentOrder.design_notes}</p>
                  </div>
                )}
              </div>
            ...
            <div className="rounded-lg bg-[#11141c] border border-[#1f2533] p-5 space-y-4">
              <div className="flex items-center gap-2 pb-3 border-b border-[#1f2533]">
                ...
                <h3 className="text-xs font-bold uppercase tracking-wider text-slate-300">Carrier &amp; Dispatch</h3>
              </div>
              {currentOrder.delivery_driver ? (
                <div className="space-y-3 text-xs">
                  ...
                  {currentOrder.tracking_number && (
                    <div className="p-3 rounded-md bg-[#090b10] border border-[#1f2533]">
                      ...
                      <span className="text-xs font-mono font-bold text-blue-400 mt-0.5 block">{currentOrder.tracking_number}</span>
                    </div>
                  )}
                ...
                <div className="py-8 text-center text-slate-500 text-xs">
                  ...
                  <p className="text-[11px] text-slate-500 mt-1">Courier details and tracking bill assigned upon packaging completion.</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="rounded-lg bg-[#11141c] border border-[#1f2533] p-5 space-y-4">
            <div className="flex items-center justify-between pb-3 border-b border-[#1f2533]">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-xs font-bold uppercase tracking-wider text-slate-300">Verification &amp; Custody Ledger</h3>
              </div>
