...
import { ArrowRight, Clock, Cog, CheckCircle2, Package, Truck, CheckCheck, Eye, RefreshCw, Search, LayoutGrid, List, Check } from 'lucide-react';
...
export default function WarehouseAdmin() {
  ...
  useEffect(() => {
    const handleUpdate = (updatedOrder) => {
      ...
      setRecentNotification({ order_number: updatedOrder.order_number, status: updatedOrder.status, client: updatedOrder.client_name });
      setTimeout(() => setRecentNotification(null), 3500);
    };
    const handleCreate = (newOrder) => {
      setOrders((prev) => [newOrder, ...prev]);
      setRecentNotification({ order_number: newOrder.order_number, status: 'CONFIRMED', client: newOrder.client_name, isNew: true });
      setTimeout(() => setRecentNotification(null), 3500);
    };
  ...
  const filteredOrders = orders.filter((o) => {
    ...
    return o.order_number.toLowerCase().includes(term) || o.client_name.toLowerCase().includes(term) || o.product.toLowerCase().includes(term);
  });
  ...
  return (
    <div className="space-y-5 pb-20">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-[#1f2533]">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-lg font-bold text-white tracking-tight">Shop Floor Operations &amp; Pipeline</h1>
            <span className="text-[11px] font-mono px-2 py-0.5 rounded bg-[#161a24] text-slate-300 border border-[#1f2533]">{orders.length} ACTIVE JOBS</span>
          </div>
          <p className="text-xs text-slate-400 mt-0.5">Real-time routing across CNC, print beds, quality control, and carrier loading docks.</p>
        </div>
        <div className="flex items-center gap-2.5">
          ...
          <div className="flex items-center bg-[#11141c] border border-[#1f2533] rounded-md p-0.5">
            <button onClick={() => setViewMode('board')} className={`p-1.5 rounded text-xs transition-colors ${viewMode === 'board' ? 'bg-[#1f2533] text-white' : 'text-slate-400 hover:text-white'}`} title="Board View">
              <LayoutGrid size={14} />
            </button>
            <button onClick={() => setViewMode('list')} className={`p-1.5 rounded text-xs transition-colors ${viewMode === 'list' ? 'bg-[#1f2533] text-white' : 'text-slate-400 hover:text-white'}`} title="List View">
              <List size={14} />
            </button>
          </div>
          <button onClick={loadOrders} title="Refresh" className="p-1.5 rounded-md bg-[#11141c] border border-[#1f2533] text-slate-400 hover:text-white transition-colors">
      ...
      {/* Ticker */}
      {recentNotification && (
        <div className="px-3.5 py-2 rounded-md bg-[#161a24] border border-blue-900/50 text-blue-200 text-xs flex items-center justify-between font-mono">
          <div className="flex items-center gap-2">
      ...
      {viewMode === 'board' ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3 items-start">
          {PIPELINE_COLUMNS.map((col) => {
            ...
            return (
              <div key={col.id} className={`flex flex-col rounded-lg bg-[#0e1118] border border-[#1f2533] border-t-2 ${col.accent} min-h-[560px] overflow-hidden shadow-sm`}>
                <div className="px-3 py-2.5 border-b border-[#1f2533] bg-[#11141c] flex items-center justify-between">
                  ...
                  <span className="text-[10px] font-mono px-1.5 py-[0.05rem] rounded bg-[#181d28] text-slate-400 border border-[#1f2533]">{colOrders.length}</span>
                </div>
                <div className="p-2 space-y-2 flex-1 overflow-y-auto max-h-[calc(100vh-250px)]">
                  {colOrders.length === 0 ? (
                    <div className="py-12 text-center text-slate-600 text-[11px]">Queue clear</div>
                  ) : (
                    colOrders.map((order) => {
                      const isWorking = actionLoadingId === order.id;
                      return (
                        <div key={order.id} className="rounded-md bg-[#131720] hover:bg-[#181d28] border border-[#1f2533] p-3 space-y-2.5 transition-colors shadow-sm text-xs group">
                          <div className="flex items-center justify-between">
                            <span className="font-mono font-bold text-white text-xs tracking-tight">{order.order_number}</span>
                            <div className="flex items-center gap-1">
                              {order.priority && order.priority !== 'NORMAL' && (
                                <span className="text-[9px] font-mono font-bold px-1 rounded bg-rose-950/60 text-rose-300 border border-rose-800/40">{order.priority}</span>
                              )}
                              <button onClick={() => setDetailOrder(order)} title="View Full Specs" className="p-0.5 rounded text-slate-500 hover:text-slate-300 transition-colors">
                                <Eye size={13} />
                              </button>
                            </div>
                          </div>
                          <div>
                            <div className="font-semibold text-slate-200 truncate">{order.client_name}</div>
                            <div className="text-[11px] text-slate-400 mt-0.5 truncate">{order.product}</div>
              ...
              {filteredOrders.map((order) => {
                ...
                return (
                  <tr key={order.id} className="hover:bg-[#161a24] transition-colors">
                    <td className="py-2.5 px-3 font-mono font-bold text-white">{order.order_number}</td>
                    <td className="py-2.5 px-3 text-slate-200 font-medium">{order.client_name}</td>
                    ...
                    <td className="py-2.5 px-3 text-right">
                      <button onClick={() => setDetailOrder(order)} className="p-1 rounded bg-[#161a24] hover:bg-slate-700 text-slate-400 hover:text-white">
                        <Eye size={13} />
                      </button>
                    </td>
                  </tr>
      ...
      {dispatchOrder && <DispatchModal isOpen={!!dispatchOrder} order={dispatchOrder} onClose={() => setDispatchOrder(null)} onConfirm={handleConfirmDispatch} />}
      {detailOrder && <OrderDetailModal isOpen={!!detailOrder} order={detailOrder} onClose={() => setDetailOrder(null)} />}
    </div>
  );
}
