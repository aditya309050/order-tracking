...
import { Package, Clock, Cog, CheckCircle2, Truck, CheckCheck, MapPin, Phone, ShieldCheck, RefreshCw, FileText, Check, Copy, Printer } from 'lucide-react';
...
export default function ClientPortal() {
  ...
  useEffect(() => {
    if (selectedOrder) subscribeToOrder(selectedOrder.order_number);
    ...
    return () => {
      if (selectedOrder) unsubscribeFromOrder(selectedOrder.order_number);
  ...
  const currentStepIndex = selectedOrder ? TRACKING_STEPS.findIndex((s) => s.key === selectedOrder.status) : -1;

  const activityMap = {};
  if (selectedOrder && selectedOrder.timeline) {
    for (const act of selectedOrder.timeline) activityMap[act.status_to] = act;
  }
    ...
    <div className="max-w-4xl mx-auto space-y-6 pb-20">
      ...
      <div className="rounded-lg bg-[#11141c] border border-[#1f2533] p-5 sm:p-6 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <h1 className="text-xl sm:text-2xl font-bold text-white tracking-tight">Welcome, {user?.name || 'Valued Client'}</h1>
          <p className="text-xs text-slate-400 mt-0.5">You are securely authenticated. Below are the work orders registered to your organization.</p>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={loadClientOrders} className="p-1.5 rounded-md bg-[#161a24] hover:bg-slate-800 text-slate-300 border border-[#1f2533] transition-colors" title="Refresh">
          ...
          {orders.map((order) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-md font-mono text-xs font-semibold transition-colors ${
                selectedOrder?.id === order.id ? 'bg-blue-600 text-white shadow-sm' : 'bg-[#11141c] text-slate-400 hover:text-white border border-[#1f2533]'
              }`}
            >
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-5 border-b border-[#1f2533]">
              <div>
                ...
                <div className="flex items-center gap-3 mt-1">
                  <h2 className="text-2xl font-bold font-mono text-white tracking-tight">{selectedOrder.order_number}</h2>
              ...
              <div className="flex sm:flex-col sm:items-end justify-between items-center gap-2">
                <div className="text-left sm:text-right">
                  ...
                  <span className="text-sm font-semibold text-slate-200 font-mono">{selectedOrder.estimated_delivery || 'In Progress'}</span>
                </div>
            ...
            <div className="pt-6 pb-2">
              <div className="relative">
                ...
                <div
                  ...
                  style={{ width: `${Math.min(100, (currentStepIndex / (TRACKING_STEPS.length - 1)) * 92)}%` }}
                />
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="md:col-span-2 rounded-lg bg-[#11141c] border border-[#1f2533] p-5 space-y-4">
              <div className="flex items-center justify-between pb-3 border-b border-[#1f2533]">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-xs font-bold uppercase tracking-wider text-slate-300">Your Item Specifications</h3>
                </div>
                <span className="text-[11px] font-mono text-slate-400">{selectedOrder.quantity} Units Total</span>
              </div>
              <div className="rounded-md bg-[#090b10] border border-[#1f2533] p-4 space-y-3">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <h4 className="text-sm font-semibold text-white">{selectedOrder.product}</h4>
                    <p className="text-xs text-slate-400 mt-0.5">Category: {selectedOrder.category} &bull; Dimensions: {selectedOrder.dimensions || 'Custom Size'}</p>
                  </div>
                  <div className="text-right shrink-0">
                    <span className="text-xs font-mono font-bold text-white bg-slate-800 px-2 py-0.5 rounded border border-slate-700">QTY: {selectedOrder.quantity}</span>
                  </div>
                </div>
                {selectedOrder.design_notes && (
                  <div className="pt-2 border-t border-[#1f2533] text-xs">
                    <span className="text-slate-500 text-[11px] font-medium block mb-1">Notes &amp; Finishing:</span>
                    <p className="text-slate-300 font-mono text-[11px] leading-relaxed bg-[#11141c] p-2.5 rounded border border-[#1f2533]">{selectedOrder.design_notes}</p>
                  </div>
                )}
              </div>
            ...
            <div className="rounded-lg bg-[#11141c] border border-[#1f2533] p-5 space-y-4">
              <div className="flex items-center gap-2 pb-3 border-b border-[#1f2533]">
                ...
                <h3 className="text-xs font-bold uppercase tracking-wider text-slate-300">Logistics &amp; Courier</h3>
              </div>
              {selectedOrder.delivery_driver ? (
                <div className="space-y-3 text-xs">
                  ...
                  {selectedOrder.tracking_number && (
                    <div className="p-3 rounded-md bg-[#090b10] border border-[#1f2533]">
                      ...
                      <span className="text-xs font-mono font-bold text-blue-400 mt-0.5 block">{selectedOrder.tracking_number}</span>
                    </div>
                  )}
                ...
                <div className="py-8 text-center text-slate-500 text-xs">
                  ...
                  <p className="text-[11px] text-slate-500 mt-1">Driver assignment details appear here once packaging is complete.</p>
                </div>
              )}
            </div>
          </div>
