  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (from && from !== '/login') {
        ...
        router.replace('/officeadmin');
      } else if (user.role === 'WAREHOUSE_ADMIN') {
        router.replace('/warehouseadmin');
      } else {
    ...
    <div className="min-h-[85vh] flex items-center justify-center p-4">
      <div className="w-full max-w-md space-y-5">
        ...
        <div className="text-center space-y-2">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-white font-mono">VANGUARD OMS</h1>
            <p className="text-xs text-slate-400 mt-0.5">Production Operations, Warehouse Dispatch &amp; Client Portal</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 p-1 rounded-lg bg-[#11141c] border border-[#1f2533] text-xs font-semibold">
          <button
            ...
            className={`py-2 rounded-md transition-colors flex items-center justify-center gap-1.5 ${
              activeTab === 'client' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-400 hover:text-slate-200'
            }`}
          >
            <User size={13} />
        ...
        <div className="rounded-xl bg-[#11141c] border border-[#1f2533] p-6 shadow-xl space-y-5 text-xs">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            >
              <span>
                {submitting ? 'Authenticating...' : activeTab === 'client' ? 'Sign In to Client Portal' : 'Sign In to Operations Console'}
              </span>
              <ArrowRight size={13} />
            </button>
          </form>
