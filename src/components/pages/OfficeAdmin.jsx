...
import { Search, Copy, ExternalLink, Eye, Check, Package, RefreshCw, Key, Database, Plus } from 'lucide-react';
...
export default function OfficeAdmin() {
  ...
  const loadOrders = async () => {
    try {
      setLoading(true);
      const res = await fetchOrders({ status: selectedStatus, search: searchTerm });
  ...
  const handleCopyLink = (orderNumber) => {
    navigator.clipboard.writeText(`${window.location.origin}/track/${orderNumber}`);
...
Password: ${order.client_password || 'pass1234'}`;
  ...
  return (
    <div className="space-y-5 pb-20">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-[#1f2533]">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-lg font-bold text-white tracking-tight">Order Intake &amp; Client Accounts</h1>
            <span className="text-[11px] font-mono px-2 py-0.5 rounded bg-[#161a24] text-slate-300 border border-[#1f2533]">
              {orders.length} RECORDS
            </span>
          </div>
          <p className="text-xs text-slate-400 mt-0.5">Log work orders, provision client credentials, and monitor customer order lifecycles.</p>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={loadOrders} title="Refresh" className="p-2 rounded-md bg-[#11141c] border border-[#1f2533] text-slate-400 hover:text-white transition-colors">
        ...
        <div className="flex items-center gap-1 overflow-x-auto pb-1 sm:pb-0 text-xs">
          <button
            ...
            className={`px-2.5 py-1 rounded text-[11px] font-medium transition-colors ${
              selectedStatus === 'ALL' ? 'bg-slate-700 text-white font-semibold' : 'bg-[#11141c] text-slate-400 hover:text-slate-200 border border-[#1f2533]'
            }`}
          >
            All
          </button>
          {Object.entries(STATUS_CONFIG).map(([key, conf]) => (
            <button
              ...
              className={`px-2 py-1 rounded text-[11px] font-medium whitespace-nowrap transition-colors ${
                selectedStatus === key ? 'bg-blue-950 text-blue-300 border border-blue-800/60 font-semibold' : 'bg-[#11141c] text-slate-400 hover:text-slate-200 border border-[#1f2533]'
              }`}
            >
              {conf.shortLabel}
            </button>
          ...
          <table className="w-full text-left text-xs border-collapse">
            ...
            <tbody className="divide-y divide-[#1f2533]/80">
              {orders.length === 0 ? (
                ...
                orders.map((order) => (
                  <tr key={order.id} className="hover:bg-[#161a24] transition-colors group">
                    <td className="py-2.5 px-3 whitespace-nowrap font-mono">
                      <div className="flex items-center gap-1.5">
                        <span className="font-bold text-white text-xs">{order.order_number}</span>
                        {order.priority && order.priority !== 'NORMAL' && (
                          <span className="text-[9px] font-bold px-1 rounded bg-rose-950/60 text-rose-300 border border-rose-800/40 uppercase">{order.priority}</span>
                        )}
                      </div>
                    </td>
                    ...
                    <td className="py-2.5 px-3">
                      <div className="text-slate-200 font-medium truncate max-w-xs">{order.product}</div>
                    ...
                    <td className="py-2.5 px-3 whitespace-nowrap">
                      <div className="flex items-center gap-1.5">
                        <div className="leading-tight">
                          ...
                          <span className="font-mono text-[10px] text-slate-500 block">PIN: {order.client_password || 'pass1234'}</span>
                        </div>
                        <button
                          onClick={() => handleCopyCredentials(order)}
                          title="Copy Client Login ID & Password"
                    ...
                    <td className="py-2.5 px-3 whitespace-nowrap text-[11px] font-mono text-slate-400">{order.estimated_delivery || 'Not set'}</td>
                    <td className="py-2.5 px-3 whitespace-nowrap text-right">
                      <div className="flex items-center justify-end gap-1">
                        <button onClick={() => handleViewOrder(order)} title="View Ledger & Specs" className="p-1 rounded bg-[#161a24] hover:bg-slate-700 text-slate-400 hover:text-white transition-colors">
                          <Eye size={13} />
                        </button>
                        <button onClick={() => handleCopyLink(order.order_number)} title="Copy Client Tracking URL" className="p-1 rounded bg-[#161a24] hover:bg-slate-700 text-slate-400 hover:text-white transition-colors">
                          {copiedId === order.order_number ? <Check size={13} className="text-emerald-400" /> : <Copy size={13} />}
                        </button>
                        <Link href={`/track/${order.order_number}`} target="_blank" title="Open Client Portal" className="p-1 rounded bg-[#161a24] hover:bg-slate-700 text-slate-400 hover:text-white transition-colors">
      ...
      {selectedOrder && <OrderDetailModal isOpen={isDetailOpen} onClose={() => setIsDetailOpen(false)} order={selectedOrder} />}
    </div>
  );
}
