...
export default function DispatchModal({ isOpen, onClose, order, onConfirm }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
      <div className="relative w-full max-w-md bg-[#11141c] border border-[#1f2533] rounded-lg shadow-2xl overflow-hidden">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-500">
                {order.order_number} &bull; {order.client_name}
              </p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-3 text-xs">
          <div>
            <label className="block text-[11px] font-medium text-slate-300 mb-1">Driver / Courier Carrier Name</label>
            <input
              type="text"
          ...
          <div>
            <label className="block text-[11px] font-medium text-slate-300 mb-1">Driver Direct Phone Contact</label>
            <input
              type="text"
          ...
          <div>
            <label className="block text-[11px] font-medium text-slate-300 mb-1">Waybill / BOL Number (Optional)</label>
            <input
              type="text"
          ...
          <div>
            <label className="block text-[11px] font-medium text-slate-300 mb-1">Dispatch Verification Note</label>
            <textarea
              rows="2"
          ...
          <div className="flex items-center justify-end gap-2 pt-2.5 border-t border-[#1f2533]">
            <button type="button" onClick={onClose} className="px-3 py-1.5 rounded text-xs text-slate-400 hover:text-white">
              Cancel
            </button>
            <button
              type="submit"
