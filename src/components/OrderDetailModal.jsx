...
export default function OrderDetailModal({ isOpen, onClose, order }) {
  ...
  const trackingUrl =
    typeof window !== 'undefined' ? `${window.location.origin}/track/${order.order_number}` : `/track/${order.order_number}`;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm overflow-y-auto">
      <div className="relative w-full max-w-3xl bg-[#11141c] border border-[#1f2533] rounded-lg shadow-2xl overflow-hidden my-8">
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-[#1f2533] bg-[#0e1118]">
          <div className="flex items-center gap-3">
            ...
            {order.priority && order.priority !== 'NORMAL' && (
              <span className="text-[9px] font-mono font-bold px-1.5 py-[0.1rem] rounded bg-rose-950/60 text-rose-300 border border-rose-800/40 uppercase">
                {order.priority}
              </span>
            )}
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={onClose} className="p-1 rounded text-slate-500 hover:text-white hover:bg-slate-800 transition-colors ml-1">
              <X size={15} />
            </button>
          </div>
        </div>
        ...
        <div className="p-5 space-y-5 max-h-[75vh] overflow-y-auto text-xs">
              ...
              <div className="space-y-1.5 text-xs">
                ...
                {order.design_notes && (
                  <div className="pt-1 text-[11px]">
                    ...
                    <p className="text-slate-300 font-mono bg-[#11141c] p-2 rounded border border-[#1f2533]">{order.design_notes}</p>
                  </div>
                )}
              </div>
            </div>
          ...
          {/* Ledger History */}
          <div>
            <h4 className="text-[10px] font-mono font-bold uppercase tracking-wider text-slate-500 mb-2">Chronological Audit Trail</h4>
